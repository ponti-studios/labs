...
export function AllTrackersPage() {
  ...
  useEffect(() => {
    const fetchTrackers = async () => {
      setLoading(true);
      const { data, error: dbError } = await supabase.from("trackers").select("*");
  ...
  if (error) {
    return <div className="text-center py-10 text-red-500">Error loading trackers: {error}</div>;
  }
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {trackers.map((tracker) => {
          ...
          const selectedTheme =
            COLOR_THEMES.find((theme) => theme.value === tracker.color_theme) || COLOR_THEMES[0];
          const selectedType =
            PERSONALITY_TYPES.find((type) => type.value === tracker.card_type) ||
            PERSONALITY_TYPES[0];
