...
export function CardRatingsPage() {
  ...
  const [ratings, setRatings] = useState<Rating[]>([
    ...
    },
    {
      ...
      name: "Taylor",
      verdict: "dump",
      comment: "Too stubborn for my taste. You deserve someone who listens more.",
    ...
    },
    {
      ...
      name: "Jordan",
      verdict: "stay",
      comment: "You two are perfect together! Love how they surprise you with gifts.",
      date: new Date(),
    },
  ]);
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        const percentage = total > 0 ? Math.round((currentStayCount / total) * 100) : 0;
  ...
  const submitRating = () => {
    ...
    setRatings((prev) =>
      [...prev, newRatingObj].sort((a, b) => b.date.getTime() - a.date.getTime()),
    );
      ...
      <div className="bg-gray-50 p-4 sm:p-6 rounded-lg border border-gray-200">
        ...
        <div className="space-y-4 mb-6">
          {loadingResults ? (
            ...
            <div className="text-center py-4 text-red-500">Error: {errorResults}</div>
          ) : totalRatingsFetched > 0 ? (
            <>
              <div className="bg-white p-4 rounded-lg border border-gray-200">
                <h3 className="text-lg font-semibold text-center mb-2">Overall Verdict</h3>
                <div className="flex justify-center mb-4">
                  ...
                  >
                    {stayPercentageFetched >= 50 ? "Stay Together" : "Consider Breaking Up"}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Progress value={stayPercentageFetched} className="h-2 bg-green-500" />

                  <div className="flex justify-between text-sm">
                    <span className="font-medium">Dump</span>
                    <span>{100 - stayPercentageFetched}%</span>
                  </div>
                  <Progress value={100 - stayPercentageFetched} className="h-2 bg-red-500" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="bg-white p-3 rounded-lg border border-gray-200 text-sm">
                  ...
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="font-medium text-green-600">{stayCountFetched}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Dump Votes</span>
                    <span className="font-medium text-red-600">{dumpCountFetched}</span>
                  </div>
                </div>
              </div>
        ...
        <h4 className="text-md font-semibold mb-3">Individual Comments ({ratings.length})</h4>
        <div className="space-y-4 max-h-[400px] sm:max-h-[500px] overflow-y-auto pr-2">
          {ratings.length > 0 ? (
            ratings.map((rating) => (
              <div key={rating.id} className="p-3 bg-white rounded-lg border border-gray-200">
