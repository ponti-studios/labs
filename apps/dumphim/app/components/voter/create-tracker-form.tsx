    ...
    <form onSubmit={handleSubmit} className="pokemon-card space-y-4 p-1">
      ...
      <div className="pokemon-card-content space-y-4">
        <div className="space-y-2">
          <label htmlFor="tracker-photo" className="block text-sm font-bold text-gray-800">
        ...
        <div>
          <label htmlFor="tracker-name" className="block text-sm font-bold text-gray-800">
            Their Name
          </label>
          <input
            id="tracker-name"
        ...
        <div className="space-y-2">
          <div className="flex items-center">
            ...
            <h4 className="block text-sm font-bold text-gray-800">The Good (Pros)</h4>
          </div>
          {pros.map((pro, index) => (
            <div key={prosKeys.current[index]} className="flex items-center mb-2">
              <input
                type="text"
                placeholder={index === 0 ? "Pro #1 (required)" : `Pro #${index + 1} (optional)`}
                value={pro}
        ...
        <div className="space-y-2">
          <div className="flex items-center">
            ...
            <h4 className="block text-sm font-bold text-gray-800">The Bad (Cons)</h4>
          </div>
          {cons.map((con, index) => (
            <div key={consKeys.current[index]} className="flex items-center mb-2">
              <input
                type="text"
                placeholder={index === 0 ? "Con #1 (required)" : `Con #${index + 1} (optional)`}
                value={con}
        ...
        <div className="pokemon-card-stats">
          ...
          <div className="hp-bar-container">
            <div className="hp-bar-glow hp-bar-neutral hp-bar-animated" style={{ width: "50%" }} />
          </div>
          <p className="mt-2 text-xs italic">HP will be calculated based on friends' votes!</p>
        </div>
        <div className="flex justify-end space-x-3 pt-2">
          ...
          <button
            type="submit"
            disabled={!name || !pros[0] || !cons[0] || isSubmitting || !!uploadError}
