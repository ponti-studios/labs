...
export function CardRatingsPage() {
  ...
  const [localSocialVotes, setLocalSocialVotes] = useState<SocialVote[]>(votes);
  ...
  const localStayCount = localSocialVotes.filter((v) => v.value === "stay").length;
  const localDumpCount = localSocialVotes.filter((v) => v.value === "dump").length;
  const localTotal = localSocialVotes.length;
  ...
  const submitRating = async () => {
    if (newRating.name && newRating.verdict) {
      ...
      const optimisticSocialVote: SocialVote = {
        id: `temp-${Date.now()}`,
