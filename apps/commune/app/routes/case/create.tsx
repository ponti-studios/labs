...
import { COLOR_THEMES, CardThemePicker } from "~/components/verdict/card-theme-picker";
...
} from "../../components/verdict/personality-type-picker";
...
export function CardCreator() {
  ...
  const handleSaveCase = async () => {
    ...
    if (!user) {
      setSaveError("You must be logged in to create a case.");
      return;
    }
    ...
    try {
      ...
      const data = await response.json();
      setSaveSuccess(`Case created for ${data.name ?? cardData.name}.`);
      console.log("Case saved:", data);
    } catch (err: any) {
      console.error("Error saving case:", err);
      setSaveError(`Error saving case: ${err.message}`);
    } finally {
      setIsSaving(false);
    }
  };
