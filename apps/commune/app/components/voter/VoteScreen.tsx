...
interface VoteScreenProps {
  tracker: SocialTrackerParsed;
  votes: SocialVote[];
  onBack: () => void;
  onVoteCasted: (trackerId: string, updatedVotes: SocialVote[]) => void;
  fetcher: any;
}
...
const VoteScreen: React.FC<VoteScreenProps> = ({
  ...
  const { user } = useAuth();
  const [currentVotes, setCurrentVotes] = useState<SocialVote[]>(votes);
  ...
  const handleVote = async (value: "stay" | "dump") => {
    ...
    const newVotePayload: Omit<SocialVote, "id" | "createdAt" | "updatedAt"> = {
      value,
      fingerprint,
      userId: userId ?? null,
    ...
    const optimisticVote: SocialVote = {
      ...newVotePayload,
      id: `temp-${Date.now()}`,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  return (
    <div className="space-y-6 p-2 md:p-4 max-w-2xl mx-auto">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-600 dark:text-gray-300 px-4 text-lg">{tracker.description}</p>
      </div>
