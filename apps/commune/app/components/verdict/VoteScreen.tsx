...
interface VoteScreenProps {
  caseRecord: RelationshipCaseParsed;
  votes: RelationshipVerdict[];
  onBack: () => void;
  onVoteCasted: (caseId: string, updatedVerdicts: RelationshipVerdict[]) => void;
  fetcher: any;
}
...
const VoteScreen: React.FC<VoteScreenProps> = ({
  ...
  useEffect(() => {
    if (fetcher.data?.success && fetcher.data?.vote) {
      ...
      onVoteCasted(caseRecord.id, finalVotesArray);
    }
  }, [fetcher.data]);
