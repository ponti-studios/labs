    ...
    for (let x = 0; x < GRID; x++) {
      ...
      cells.push(
        <button
          ...
          className={[
            "flex items-center justify-center bg-background border-r border-b border-border cursor-pointer transition-colors",
...
function CommandBuilder({
  ...
  const btnCls = (extra = "") =>
    `h-12 w-14 rounded border font-mono text-base font-semibold transition-colors
     ${
       disabled
         ? "border-border text-muted-foreground/30 cursor-not-allowed bg-muted/10"
