  ...
  return (
    <section className="section detail-hero">
      ...
      <div className="flex min-w-0 flex-col gap-4">
        ...
        {metadata ? <div className="body-2 text-muted-foreground max-w-2xl">{metadata}</div> : null}
        <p className="body-1 text-foreground max-w-2xl">{summary}</p>
      </div>
    </section>
  );
}
    ...
    <nav aria-label={ariaLabel} className="section section-compact">
      <div className="detail-navigation">
        {previous ? (
          ...
          >
            <span className="body-4 text-muted-foreground group-hover:text-accent transition-colors motion-reduce:transition-none">
              ← {previous.label}
            </span>
            <span className="heading-3 text-foreground group-hover:text-accent transition-colors motion-reduce:transition-none">
              {previous.title}
            </span>
          </Link>
        ) : null}
