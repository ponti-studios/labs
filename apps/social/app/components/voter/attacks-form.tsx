...
interface AttacksFormProps {
  attacks: SocialTrackerParsed["attacks"];
...
export function AttacksForm({ attacks, onAttackChange }: AttacksFormProps) {
  ...
  return (
    <div data-testid="attacks" className="space-y-4">
      {attacks.map((attack: SocialTrackerParsed["attacks"][number], index: number) => (
        <Card key={attack.name} className="border-gray-300">
          <CardContent className="pt-6 space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor={`attackDamage-${index}`} className="text-gray-700">
              ...
              <div className="flex items-center gap-4">
                <Slider
                  ...
                  onValueChange={(value) => onAttackChange(index, "damage", String(value[0]))}
                  className="flex-1"
                />
                <span className="w-12 text-center font-bold">{attack.damage}</span>
              </div>
            </div>
          </CardContent>
        </Card>
