...
function Robot({ initialPos, isLost, scentPoints, onRobotLost, gridSize, otherRobots }: RobotProps) {
  ...
  const move = () => {
    ...
    switch (orientation) {
      case 'N': newGridPos.row -= 1; newTop -= STEP_SIZE; break
      case 'S': newGridPos.row += 1; newTop += STEP_SIZE; break
      case 'E': newGridPos.col += 1; newLeft += STEP_SIZE; break
      case 'W': newGridPos.col -= 1; newLeft -= STEP_SIZE; break
    }

    if (newGridPos.row < 1 || newGridPos.row > gridSize || newGridPos.col < 1 || newGridPos.col > gridSize) {
...
export default function RedBadger() {
  ...
  return (
    <div>
      ...
      <p>A Mars robot simulation. Enter robot position (e.g., "1 1 S" for row 1, col 1, facing South).</p>
      ...
      <div className="card" style={{ marginTop: '1rem' }}>
        ...
        <ul style={{ margin: 0, paddingLeft: '1.5rem' }}>
          <li><strong>Position:</strong> "row col direction" (e.g., "1 1 N")</li>
          <li><strong>Commands:</strong> F = forward, L = turn left, R = turn right</li>
          <li><strong>Orientation:</strong> N (north/up), E (east/right), S (south/down), W (west/left)</li>
          <li><strong>Lost robots:</strong> Leave a scent that prevents future robots from falling off at that point</li>
        </ul>
        {lostCount > 0 && (
          <p style={{ marginTop: '0.5rem', color: '#666' }}>
            Robots lost: {lostCount}
          </p>
        )}
      </div>
    </div>
