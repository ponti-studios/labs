...
export async function loader({ request }: LoaderFunctionArgs) {
  try {
    ...
    if (deathAverages.length > 6) {
      ...
      const meanDeaths = deathAverages.reduce((sum, val) => sum + val, 0) / deathAverages.length;

      // Death seasonality strength as coefficient of variation
      deathSeasonalityStrength = meanDeaths > 0 ? (maxDeaths - minDeaths) / meanDeaths : 0;

      // Find death peak and trough months
      deathPeakMonth = patterns.findIndex((p) => p.averageDeaths === maxDeaths) + 1;
      deathTroughMonth = patterns.findIndex((p) => p.averageDeaths === minDeaths) + 1;
    }
    ...
    const detectPatterns = (patterns: SeasonalPattern[]) => {
      ...
      const winterDeathAvg =
        winterMonths.reduce((sum, month) => sum + patterns[month - 1].averageDeaths, 0) / 3;
      const overallDeathAvg = patterns.reduce((sum, p) => sum + p.averageDeaths, 0) / 12;
      ...
      const summerDeathAvg =
        summerMonths.reduce((sum, month) => sum + patterns[month - 1].averageDeaths, 0) / 3;
    ...
    const analysis: SeasonalAnalysis = {
      ...
      peakMonth,
      troughMonth,
      deathSeasonalityStrength: Math.round(deathSeasonalityStrength * 1000) / 1000,
      deathPeakMonth,
      deathTroughMonth,
      patterns,
    };
