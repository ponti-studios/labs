...
import { countPendingArticlesForGame } from "~/lib/what/server/articles.server";
import { getGameBySlug } from "~/lib/what/server/games.server";
import { countInventoryForRange, loadPuzzleForDate } from "~/lib/what/server/puzzles.server";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (!game) {
    return Response.json(
      { health: "DEGRADED", error: `game not found: ${DEFAULT_WHAT_GAME_SLUG}` },
      {
        status: 500,
      },
    );
