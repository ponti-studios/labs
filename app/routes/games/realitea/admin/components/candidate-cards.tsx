...
import { explainGenerateReason } from "~/lib/realitea/admin/generate-copy";
import type { AdminGenerationCandidate } from "~/lib/realitea/admin/inventory";
    ...
    <Card>
        ...
        <CardTitle className={styles.header}>
          ...
          >
            {candidate.candidate.answer
              ...
              .split("")
              .map((letter, index) => (
                <RealiTeaTile key={`${letter}-${index}`} state="correct" letter={letter} />
              ))}
          </div>
      ...
      <CardContent className="grid gap-4 pt-0 text-sm">
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          <fetcher.Form
            method="post"
            action={`/games/realitea/admin/generations/${generationId}?game=${gameSlug}`}
          >
