...
import { loadAdminInventory } from "~/lib/what/admin/inventory";
...
export function meta() {
  return [{ title: "What inventory" }, { name: "robots", content: "noindex" }];
}
...
export default function WhatAdminInventory() {
  const inventory = useLoaderData<typeof loader>();

  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 p-6">
      <Button asChild variant="ghost" size="sm" className="w-fit">
        <Link to={`/games/what/admin?game=${inventory.game.slug}`}>← Admin</Link>
      </Button>
