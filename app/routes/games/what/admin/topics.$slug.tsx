...
} from "~/lib/what/admin/articles.server";
import { getWhatAdminActor } from "~/lib/what/admin/auth";
import {
  articleStatusValues,
  isArticleStatus,
  type ArticleStatus,
} from "~/lib/what/article-status";
...
export function meta() {
  return [{ title: "What articles" }, { name: "robots", content: "noindex" }];
}
...
export default function WhatAdminTopicArticles() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 p-6">
      <Button asChild variant="ghost" size="sm" className="w-fit">
        <Link to="/games/what/admin/topics">← Topics</Link>
      </Button>
      ...
      <div className="flex flex-wrap gap-2">
        {(["all", ...articleStatusValues] as const).map((value) => {
          const href =
            value === "all"
              ? `/games/what/admin/topics/${topic.slug}`
              : `/games/what/admin/topics/${topic.slug}?status=${value}`;
