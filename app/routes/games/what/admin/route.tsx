...
import { formatUsd } from "~/lib/what/admin/format";
import { loadAdminOverview } from "~/lib/what/admin/inventory";
...
export function meta() {
  return [{ title: "What admin" }, { name: "robots", content: "noindex" }];
}
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (!overview) {
    throw Response.json({ error: `No active What topic found` }, { status: 404 });
  }
  return overview;
}

export default function WhatAdminOverview() {
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Button asChild variant="outline">
              <Link to="/games/what/admin/topics">Articles</Link>
            </Button>
            <Button asChild variant="outline">
              <Link to="/games/what/admin/costs">Costs</Link>
            </Button>
            <Button asChild>
              <Link to={`/games/what/admin/generate?game=${overview.game.slug}`}>Generate</Link>
            </Button>
          </div>
        }
      />
      ...
      <section className="flex flex-col gap-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <Button asChild variant="outline" size="sm">
            <Link to={`/games/what/admin/inventory?game=${overview.game.slug}`}>View all</Link>
          </Button>
        </div>
