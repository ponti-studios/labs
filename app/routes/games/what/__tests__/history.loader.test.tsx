...
describe("What history route loader", () => {
  beforeEach(() => {
  ...
  it("returns signedIn: false with a loginUrl when there's no session, without querying history", async () => {
    ...
    const response = await loader(createLoaderArgs("https://labs.ponti.io/games/what/history"));
    const payload = await response.json();

    expect(payload.signedIn).toBe(false);
    expect(payload.loginUrl).toContain("labs.ponti.io%2Fgames%2Fwhat%2Fhistory");
  ...
  it("parses the page search param", async () => {
    ...
    const { loader } = await import("../history");
    await loader(createLoaderArgs("https://labs.ponti.io/games/what/history?page=3"));
  ...
  it("falls back to page 1 for an invalid page param", async () => {
    ...
    const { loader } = await import("../history");
    await loader(createLoaderArgs("https://labs.ponti.io/games/what/history?page=not-a-number"));
