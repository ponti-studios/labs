...
STUB_LOGIN_URL.searchParams.set("next", "https://labs.ponti.io/games/what");
...
// via `mockAttempt`.
type MockAttempt = { guesses: WhatGuess[]; status: GameStatus } | null;
...
async function renderRoute(initial: { puzzle?: PublicGamesPuzzle } = {}) {
  ...
  const RoutesStub = createRoutesStub([
    ...
    {
      id: "routes/api.games.what.guess",
      path: "/api/games/what/guess",
...
async function finishTileReveal() {
  for (let step = 0; step < WHAT_ANSWER_LENGTH + 3; step += 1) {
    await act(async () => {
...
function getTextboxes() {
  return Array.from({ length: WHAT_ANSWER_LENGTH }, (_, i) =>
 ...
 *    - CSS animation "what-row-shake" triggers on the rows
 *
...
describe("WhatRoute", () => {
  ...
  it("renders Black Lacquer tile and keyboard presentation states", async () => {
    ...
    expect(screen.getByRole("button", { name: "Q" })).toHaveClass("what-key");
  ...
  it("does not capture typing from a focused text input", async () => {
    ...
    expect(getTextboxValues()).toEqual(Array.from({ length: WHAT_ANSWER_LENGTH }, () => ""));
    input.remove();
  });
  ...
  it("reveals the clue only when one guess remains", async () => {
    ...
    const seededGuesses: WhatGuess[] = seededWords.map((word) => ({
      word,
