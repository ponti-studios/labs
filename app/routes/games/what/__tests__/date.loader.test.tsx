...
describe("What date route loader", () => {
  beforeEach(() => {
  ...
  it("400s on a malformed date param without touching the puzzle lookup", async () => {
    ...
    await expect(
      loader(createLoaderArgs("https://labs.ponti.io/games/what/not-a-date", "not-a-date")),
  ...
  it("400s when the date param is missing entirely", async () => {
    ...
    await expect(
      loader(createLoaderArgs("https://labs.ponti.io/games/what/")),
    ).rejects.toMatchObject({ status: 400 });
  });
  ...
  it("404s when no puzzle exists for that date", async () => {
    ...
    await expect(
      loader(createLoaderArgs("https://labs.ponti.io/games/what/2026-05-20", "2026-05-20")),
  ...
  it("returns signedIn: false for an anonymous visitor, with the puzzle and no attempt", async () => {
    ...
    const response = await loader(
      createLoaderArgs("https://labs.ponti.io/games/what/2026-05-20", "2026-05-20"),
    );
