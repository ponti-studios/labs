...
import { type PublicGamesPuzzle } from "~/lib/what";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (!envelope) {
    throw Response.json(
      ...
      { status: 404, statusText: "No WH?T puzzle found for today" },
    );
  }
...
export function ErrorBoundary({ error }: Route.ErrorBoundaryProps) {
  ...
  let message = "The WH?T puzzle couldn't load. Try refreshing the page.";
  ...
  if (isOffline) {
    message = "You're offline. WH?T needs a connection to load today's puzzle.";
  }

  if (error && !isRouteErrorResponse(error)) {
    Sentry.withScope((scope) => {
      scope.setTag("game", "what");
      scope.setTag("what_surface", "daily-puzzle");
...
export default function WhatRoute() {
  ...
  // WhatGameBoard seeds its guesses once at mount (see use-game.ts), so
