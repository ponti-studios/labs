...
import type { GameStatus } from "~/lib/what";
import { DEFAULT_WHAT_GAME_SLUG } from "~/lib/what/generation/catalog";
import { loadPuzzleHistory, type PuzzleHistoryPage } from "~/lib/what/server/history.server";
...
export function meta() {
  return [
    { title: "WH?T — History — Labyrinth" },
    { name: "description", content: "Every WH?T puzzle you've played, and how you did." },
        ...
        {shown.length === 0 ? (
          ...
          <ul className="divide-border flex flex-col divide-y overflow-y-auto">
            {shown.map((dateKey) => (
              <li key={dateKey}>
                <Link
                  to={`/games/what/${dateKey}?game=${encodeURIComponent(gameSlug)}`}
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col items-center justify-center gap-3 px-4 text-center">
      ...
      <Button asChild variant="default" className="mt-2">
        <a href="/games/what">Back to today's puzzle</a>
      </Button>
    </div>
  );
}

export default function WhatHistoryRoute() {
  ...
  if (!data.signedIn) {
    return (
      <div
        ...
        data-testid="what-history-guest"
      >
        <EmptyState
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-4 py-10">
      ...
      {history.rows.length === 0 ? (
        <EmptyState
          ...
          action={
            <Button asChild variant="default">
              <Link to={`/games/what?game=${encodeURIComponent(gameSlug)}`}>
                Play today&apos;s puzzle
              </Link>
            </Button>
          }
        ...
        <ul className="divide-border flex flex-col divide-y">
          {history.rows.map((row) => {
            ...
            return (
              <li key={row.dateKey}>
                <Link
                  to={`/games/what/${row.dateKey}?game=${encodeURIComponent(gameSlug)}`}
                  className="hover:bg-muted/50 flex items-center gap-4 rounded-md px-2 py-4 transition-colors"
                >
                  {lastGuess && (
                    <div className="what-history-mini flex shrink-0 gap-(--what-tile-gap)">
