...
const meta: Meta<typeof WhatTile> = {
  title: "What/Tile",
  component: WhatTile,
  parameters: {
    layout: "padded",
...
type Story = StoryObj<typeof WhatTile>;
  ...
  render: () => (
    <div className="flex gap-1.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <WhatTile key={i} state="empty" loading style={{ animationDelay: `${i * 100}ms` }} />
      ))}
    </div>
  ),
};
...
export const AllStates: Story = {
  name: "All States",
  render: () => {
    const states: { state: WhatTileState; letter: string; label: string }[] = [
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap gap-4">
          {states.map(({ state, letter, label }) => (
            <div key={label} className="flex flex-col items-center gap-1.5">
              <WhatTile state={state} letter={letter} />
        ...
        <div className="flex flex-wrap gap-4">
          <div className="flex flex-col items-center gap-1.5">
            <WhatTile state="empty" loading />
            <span className="text-muted-foreground text-[10px] tracking-wider uppercase">
              loading
            </span>
          </div>
          <div className="flex flex-col items-center gap-1.5">
            <WhatTile state="typed" letter="E" hasError />
            <span className="text-muted-foreground text-[10px] tracking-wider uppercase">
              error
            </span>
          </div>
