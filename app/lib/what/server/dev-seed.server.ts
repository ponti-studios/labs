...
export const WHAT_SEED_GAME_SLUG = "rhobh";

export async function requireSeedGame() {
  const game = await getGameBySlug(WHAT_SEED_GAME_SLUG);
  if (!game) {
    throw new Error(`Game not found: ${WHAT_SEED_GAME_SLUG} — run migrations first.`);
  }
  return game;
}
...
export async function ensureSeedPuzzle(
  ...
  const url = options.articleUrl ?? `https://seed.local/what/${dateKey}/${answer.toLowerCase()}`;
