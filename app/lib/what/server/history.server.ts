import type { GameStatus, WhatGuess, PuzzleAnswerType } from "../core/types";
...
// anchored to today), not by row count — What is a one-puzzle-per-day
...
export async function loadPuzzleHistory(
  ...
  return {
    rows: rows.map(({ attempt, puzzle }) => ({
      ...
      guesses: attempt.guesses as WhatGuess[],
