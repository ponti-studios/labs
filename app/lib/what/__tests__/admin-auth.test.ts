...
import { getWhatAdminActor, requireWhatAdmin, requireWhatAdminMiddleware } from "../admin/auth";

const ORIGINAL = {
  ADMIN_SECRET: process.env.ADMIN_SECRET,
  WHAT_ADMIN_EMAILS: process.env.WHAT_ADMIN_EMAILS,
...
function adminRequest() {
  ...
  return new Request("https://labs.ponti.io/games/what/admin", {
    headers: { Authorization: `Basic ${credentials}` },
  });
}

describe("requireWhatAdmin", () => {
  beforeEach(() => {
    process.env.ADMIN_SECRET = "secret";
    delete process.env.WHAT_ADMIN_EMAILS;
  ...
  it("throws 401 without Basic Auth", async () => {
    await expect(
      requireWhatAdmin(new Request("https://labs.ponti.io/games/what/admin"), "loader"),
    ).rejects.toMatchObject({ status: 401 });
  });

  it("redirects a loader without a Hominem session", async () => {
    getHominemUserMock.mockResolvedValue(null);
    await expect(requireWhatAdmin(adminRequest(), "loader")).rejects.toMatchObject({
      status: 302,
    });
  });
  ...
  it("throws 401 JSON for an action without a Hominem session", async () => {
    getHominemUserMock.mockResolvedValue(null);
    await expect(requireWhatAdmin(adminRequest(), "action")).rejects.toMatchObject({
      status: 401,
    });
  });

  it("throws 403 when the signed-in email is off the allowlist", async () => {
    process.env.WHAT_ADMIN_EMAILS = "ops@ponti.io";
  ...
  it("accepts Basic + Hominem in test without an allowlist", async () => {
    getHominemUserMock.mockResolvedValue({ id: "u1", email: "ops@ponti.io" });
    const result = await requireWhatAdmin(adminRequest(), "loader");
  ...
  it("lets local development open the console without Basic or Hominem", async () => {
    ...
    getHominemUserMock.mockResolvedValue(null);
    const result = await requireWhatAdmin(
      new Request("http://localhost:3001/games/what/admin"),
      "loader",
    );
...
describe("requireWhatAdminMiddleware", () => {
  beforeEach(() => {
    process.env.ADMIN_SECRET = "secret";
    delete process.env.WHAT_ADMIN_EMAILS;
    delete process.env.RAILWAY_ENVIRONMENT;
  ...
  it("sets the actor on context for a GET", async () => {
    ...
    await requireWhatAdminMiddleware(
      {
        context,
        params: {},
        pattern: "",
    ...
    expect(getWhatAdminActor(context).userId).toBe("u1");
  });

  it("returns 401 JSON for a POST without a Hominem session", async () => {
    getHominemUserMock.mockResolvedValue(null);
    const credentials = Buffer.from("admin:secret").toString("base64");
    const request = new Request("https://labs.ponti.io/games/what/admin", {
      method: "POST",
      headers: { Authorization: `Basic ${credentials}` },
    });
    const context = new RouterContextProvider();
    await expect(
      requireWhatAdminMiddleware(
        {
          context,
          params: {},
          pattern: "",
