...
import { normalizeGuess, WHAT_ANSWER_LENGTH } from "../core/rules";
...
export const DEFAULT_GENERATION_PROMPT_PATH = "app/lib/prompts/what-generation.md";

/** Overridable without a redeploy, mirroring WHAT_AI_MODEL — lets ops raise the budget if a model needs more room to reason. */
export function getConfiguredMaxTokens(): number {
  const raw = process.env.WHAT_MAX_TOKENS;
...
export function getConfiguredReasoningEffort(): string | undefined {
  const raw = process.env.WHAT_REASONING_EFFORT;
...
export async function generatePuzzleForGame(
  ...
  childLogger.info(
    ...
    "using What generation configuration",
  );
