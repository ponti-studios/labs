...
export async function withGenerateLock<T>(fn: () => Promise<T>): Promise<GenerateLockResult<T>> {
  ...
  try {
    const [row] = await reserved<[{ locked: boolean }]>`
      SELECT pg_try_advisory_lock(${REALITEA_GENERATE_LOCK_KEYS[0]}, ${REALITEA_GENERATE_LOCK_KEYS[1]}) AS locked
    `;
    ...
    try {
      ...
      await reserved`
        SELECT pg_advisory_unlock(${REALITEA_GENERATE_LOCK_KEYS[0]}, ${REALITEA_GENERATE_LOCK_KEYS[1]})
      `;
    }
  } finally {
    reserved.release();
