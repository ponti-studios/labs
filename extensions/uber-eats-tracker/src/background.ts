...
chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
  ...
  if (request.action === "initiateScrapingOnActiveTab") {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs[0]?.id) {
        try {
          chrome.tabs.sendMessage(tabs[0].id, { action: "startScraping" }, (_response) => {
            if (chrome.runtime.lastError) {
              console.log(`Tab message failed: ${chrome.runtime.lastError.message}`);
              sendMessageSafely({
                action: "scrapingStatusUpdate",
                status: "error",
                message: "Could not connect to page. Please reload and try again.",
              });
            }
          });
        } catch (error) {
