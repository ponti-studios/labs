...
function RestaurantsList({ restaurants }: RestaurantsListProps) {
  const sortedRestaurants = Object.entries(restaurants).sort((a, b) => b[1].total - a[1].total);
      ...
      {sortedRestaurants.map(([restaurant, info]) => (
        ...
        >
          <div className="card-body p-4">
            <h3 className="card-title text-lg font-bold text-primary">{restaurant}</h3>
            <div className="flex justify-between items-center mt-2">
              <div className="badge badge-secondary badge-lg">
                {info.visits} {info.visits === 1 ? "visit" : "visits"}
              </div>
              <div className="text-xl font-semibold text-accent">${info.total.toFixed(2)}</div>
            </div>
          </div>
        </div>
      ))}
        ...
        >
          <div className="card-body p-4">
            <div className="flex justify-between items-start">
              <h3 className="card-title text-lg font-bold text-primary">{order.restaurant}</h3>
    ...
    <div className="card bg-base-100 shadow-xl p-6 max-w-2xl mx-auto">
      <div className="space-y-6">
        <div className="stat bg-base-200 rounded-box p-4">
          ...
          <div className="stat-value text-primary">${data.total.toFixed(2)}</div>
        </div>
        ...
        <div className="stat bg-base-200 rounded-box p-4">
          <div className="stat-title text-base-content/60">Most Visited Restaurant</div>
          <div className="stat-value text-accent text-2xl">{mostVisited.name}</div>
        ...
        <div className="stat bg-base-200 rounded-box p-4">
          <div className="stat-title text-base-content/60">Highest Spend Restaurant</div>
          <div className="stat-value text-accent text-2xl">{highestSpend.name}</div>
          <div className="stat-desc text-base-content/70 text-lg mt-1">
    ...
    if (typeof chrome !== "undefined" && chrome.runtime) {
      chrome.runtime.onMessage.addListener((message) => {
        if (
          ...
          (message.type === "URL_UPDATED" && message.url.includes("ubereats.com/orders"))
        ) {
  ...
  return (
    <div className="uber-eats-tracker-app">
      ...
      <div className="controls">
        <button type="button" className="primary-button" onClick={navigateToOrdersPage}>
      ...
      {data && !loading && !error && (
        <div className="results-container">
          ...
          <div className="tab-content-container">
            ...
            {activeTab === "restaurants" && <RestaurantsList restaurants={data.restaurants} />}
