...
function getPlaylistLength(): TimeObject | null {
  ...
  const playlistContainer = document.querySelector<Element>("ytd-playlist-video-list-renderer");
...
type ContentPlaylistTimeResponse =
  | ContentPlaylistTimeSuccessResponse
  | ContentPlaylistTimeErrorResponse;
...
function setupObserver() {
  ...
  observer = new MutationObserver((mutations) => {
    const hasRelevantChanges = mutations.some((mutation) => {
      ...
      if ((mutation.target as Element).id === "content" && mutation.target.nodeName === "DIV")
        return true;
