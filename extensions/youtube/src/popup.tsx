...
function App() {
  ...
  const [timeData, setTimeData] = useState<{ formatted: string; timeObject: TimeObject } | null>(
    null,
  );
  ...
  const calculateChunks = () => {
    ...
    setChunkResult(
      `You need ${chunks} chunk${chunks !== 1 ? "s" : ""} of ${chunkMinutes} minutes each.`,
    );
  };
    ...
    <div className="w-[350px] p-4 font-sans">
      <Card className="overflow-hidden">
        ...
        <CardContent className="space-y-4 pt-4">
          <Button onClick={calculatePlaylistTime} disabled={loading} className="w-full font-medium">
          ...
          {timeData && (
            <div className="space-y-3 pt-2">
              ...
              {chunkResult && (
                <div className="text-sm mt-2 p-2 bg-secondary/10 rounded-md font-medium">
                  {chunkResult}
                </div>
              )}
            </div>
