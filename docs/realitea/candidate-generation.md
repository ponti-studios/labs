...
This is the per-date, per-game loop `generatePuzzleForGame` (`app/lib/realitea/generation/generate.server.ts`) runs. `runGenerateWindow` (`app/lib/realitea/ops.ts`) calls it once per missing date, per active game, inside a window — see [Architecture](./architecture.md) for how those calls are scheduled and circuit-broken.
