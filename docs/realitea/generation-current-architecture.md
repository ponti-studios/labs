---
title: RealiTea Generation Pipeline — Current State
project: realitea
type: reference
status: active
owner: charlesponti
...
feed (`app/lib/realitea/generation/catalog.ts:3-39`):
...
(`.github/workflows/realitea-generate.yml`) with three sequential steps:
...
sequenceDiagram
    ...
    participant Gen as realitea-generate.ts<br/>(reconcile script)
    participant Ops as ops.ts
    participant LLM as generate.server.ts<br/>+ OpenRouter
    participant Health as realitea-health-check.ts

    Cron->>Ingest: pnpm realitea:ingest
    ...
    Cron->>Gen: pnpm realitea:generate (no flags → force=false)
    ...
    Cron->>Health: pnpm realitea:health-check
...
```mermaid
flowchart TD
    A["Player requests /games/realitea (or API)"] --> B["resolveActivePuzzle(now, timeZone, gameSlug)"]
...
**File:** `app/lib/realitea/generation/ingest.server.ts`
...
- Rows are inserted via `upsertArticles` (`repository.server.ts:147-175`) with
  ...
  has a `unique()` constraint (`schema/realitea.ts:56`), so re-polling a feed
  that returns already-seen items is a no-op. New rows default to
  `status: 'pending'` (`schema/realitea.ts:63`).
- `ingestAllActiveFeeds()` (line 164) runs every active `games_topics` row's
  feed **in parallel** via `Promise.all`.
- `ensureRealiteaCatalog()` (line 171) upserts the 5-row catalog into
...
- Invoked by `scripts/realitea-ingest.ts`, run as `pnpm realitea:ingest`
  (package.json:16), the first step of the daily workflow.

### 3.2 Selection (pending-article queries)

**File:** `app/lib/realitea/server/repository.server.ts`
...
- `getPendingArticlesForGame(game, limit)` (line 189) and
  ...
  `6e6e315` ("Fix RealiTea to select the newest pending article, not the
...
**File:** `app/lib/realitea/generation/generate.server.ts`
...
3. **Gather exclusions & candidates in parallel** (lines 456-460):
   - `getRecentAnswers(game, date)` — every normalized answer used by this
     game within `game.repeatWindowDays` (default 90, `schema/realitea.ts:34`)
     days of `date`.
...
4. **Retry loop** (lines 485-548), `maxAttempts` defaulting to **3**
   ...
   - `callGenerationApi` (line 286) calls `callGenerationApiForCandidates`
     ...
     **3-5 candidates** (`generationResponseSchema`, line 90-92). Prompt is
     ...
     `app/lib/prompts/realitea-generation.md`, set in
     `ensureRealiteaCatalog`), user message contains `dateKey`,
...
5. **Publish** (lines 561-590): once a `{candidate, article}` pair wins,
   ...
   `schema/realitea.ts:222`) and call `markArticleUsed(article.id)` — this is
...
**Files:** `.github/workflows/realitea-generate.yml`, `app/lib/realitea/ops.ts`,
`scripts/realitea-generate.ts`

- The workflow (`realitea-generate.yml:5-6`) runs on `cron: "0 17 * * *"` —
...
- Steps, in order: `run-db-migrations` → `ingest-feeds`
  (`pnpm realitea:ingest`) → `generate` → `health-check`
  (`pnpm realitea:health-check`), all against the same `DATABASE_URL`.
- On a plain `schedule` trigger the `generate` step runs
  `pnpm realitea:generate` with **no flags** (workflow lines 68-70) — this
  maps to `scripts/realitea-generate.ts`'s `parseReconcileArgs()`, which
  defaults `force: false` and `daysAhead: REALITEA_READY_INVENTORY_DAYS` (7,
  from `candidate-validation.ts:7`).
- `scripts/realitea-generate.ts` (`main`, line 53) calls
...
- This runs for every active game (`getActiveGames()` in
  `realitea-generate.ts:84`) inside a Postgres advisory lock
  ...
  `concurrency: { group: realitea-generate, cancel-in-progress: false }` at
...
- `health-check` (`scripts/realitea-health-check.ts`) is the last step and
  ...
  over the next `REALITEA_READY_INVENTORY_DAYS` (7) days is below that
...
**File:** `app/lib/realitea/core/date.ts`, `app/lib/realitea/ops.ts`
...
- The 17:00 UTC cron time itself: `architecture.md:87` (an existing, possibly
  ...
  (`cron-realitea-generate.yml`, `scripts/generate-realitea-scheduled-puzzle.ts`,
  `realitea:gen`/`realitea:reconcile`) that don't match the current
  `.github/workflows/realitea-generate.yml` / `scripts/realitea-generate.ts` /
  `pnpm realitea:generate`, so treat its operational specifics as historical,
...
**File:** `app/lib/server/db/schema/realitea.ts`
...
- `articles` (line 51): `status` enum `pending | used | rejected | expired`
  ...
  above.
- `realitea_generation_runs` (line 138): one row per LLM attempt (not per
...
**File:** `app/lib/realitea/server/puzzle.server.ts`
...
**Files:** `app/routes/games/realitea/admin/generate.tsx`,
`app/lib/realitea/admin/generate.server.ts`
...
- Submitting the form posts to `/games/realitea/admin/generate/events`, which
...
- This is a **preview/score-only** path by default — `startGeneration` /
  ...
  `docs/realitea/admin-console.md`, which is itself a **design proposal**,
...
|---|---|---|
| Cron schedule | `0 17 * * *` UTC (daily) | `.github/workflows/realitea-generate.yml:6` |
...
| Article expiry | 45 days (per-game default, `articleExpiryDays`) | `schema/realitea.ts:34`, enforced by `expireStaleArticles` |
| Answer repeat window | 90 days (per-game default, `repeatWindowDays`) | `schema/realitea.ts:33` |
| Forward generation/gap-fill window (scheduled run) | 7 days ahead (`[tomorrow, tomorrow+6]`) | `REALITEA_READY_INVENTORY_DAYS = 7`, `candidate-validation.ts:7`; consumed by `resolveGenerateWindow` default `daysAhead` |
...
| Manual `workflow_dispatch` default `daysAhead` | 7 | `realitea-generate.yml:20` |
| Manual `workflow_dispatch` default `mode` | `force` | `realitea-generate.yml:9` |
...
| Default max tokens | 4000 (overridable via `REALITEA_MAX_TOKENS`) | `generate.server.ts:161,167-171` |
...
"inference"). `docs/realitea/architecture.md` describes an intended
separation of concerns but appears to predate the current file/script names
in places (it references `cron-realitea-generate.yml`,
`scripts/generate-realitea-scheduled-puzzle.ts`, `realitea:gen` /
`realitea:reconcile`, none of which exist in this codebase today — the
current names are `.github/workflows/realitea-generate.yml`,
`scripts/realitea-generate.ts`, `pnpm realitea:generate`). Its architectural
...
`docs/realitea/admin-console.md` is explicitly a **proposal document** for a
...
`REALITEA_READY_INVENTORY_DAYS = 7` (`candidate-validation.ts:7`) is used
both as the default gap-fill horizon (`ops.ts` via
`scripts/realitea-generate.ts`'s arg default) and as the health-check
threshold (`realitea-health-check.ts:40-41`). The comment on
...
5. **No visibility into staleness at serving time beyond a boolean.**
   ...
   (`computeHealthStatus`, `realitea-health-check.ts:25-45`) only look
...
| Concern | File |
|---|---|
| RSS polling / article extraction | `app/lib/realitea/generation/ingest.server.ts` |
| Feed/topic catalog | `app/lib/realitea/generation/catalog.ts` |
| Article/puzzle data access | `app/lib/realitea/server/repository.server.ts` |
| Core generation logic | `app/lib/realitea/generation/generate.server.ts` |
| Candidate validation rules | `app/lib/realitea/generation/candidate-validation.ts` |
| Window/gap-fill/force orchestration | `app/lib/realitea/ops.ts` |
| Date/timezone helpers | `app/lib/realitea/core/date.ts` |
| Puzzle serving + fallback | `app/lib/realitea/server/puzzle.server.ts` |
| DB schema | `app/lib/server/db/schema/realitea.ts` |
| Admin manual-generate route | `app/routes/games/realitea/admin/generate.tsx` |
| Admin manual-generate domain logic | `app/lib/realitea/admin/generate.server.ts` |
| Cron/reconcile entry point | `scripts/realitea-generate.ts` |
| Ingest entry point | `scripts/realitea-ingest.ts` |
| Health-check entry point | `scripts/realitea-health-check.ts` |
| Scheduled workflow | `.github/workflows/realitea-generate.yml` |
| Prior architecture narrative (partially stale filenames) | `docs/realitea/architecture.md` |
| Admin console redesign proposal (not yet built) | `docs/realitea/admin-console.md` |
