/** Seed a deterministic RealiTea history scenario for local development. */
...
const HISTORY_FEED_URL = "https://seed.local/realitea-history-feed";
const ANSWERS = ["GLASS", "PORSH", "BRAWL", "SHADE", "TOAST", "CROWN", "STORM", "FEUDS", "TWIST", "REVEL", "GOSSY", "DRAMA", "VOICE", "TEARS", "LIMOS", "CHAMP", "GLARE", "SPARK", "TRUST"];
const FILLER_GUESSES = ["ABASE", "ABASH"];
const SIX_WRONG_GUESSES = ["ABASE", "ABASH", "ABATE", "ABBEY", "ABACK", "ABAFT"];

function parseCliArgs() {
  const { values } = parseArgs({ args: process.argv.slice(2), options: { "user-id": { type: "string" } }, strict: true });
