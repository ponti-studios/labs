...
import { getDateKey } from "../app/lib/what/core/date";
import { getGameBySlug } from "../app/lib/what/server/games.server";
import { loadMostRecentPuzzle, loadPuzzleForDate } from "../app/lib/what/server/puzzles.server";
import { runScript } from "./_shared/run-script";

/**
 * Captures gallery-ready assets for the What game:
 *
 ...
 * app/data/projects.ts (public/screenshots/what-gameplay.png,
 * what-solved.png) — do not rename these without updating that file.
 * `solve` records the whole sequence as a video instead (not wired into the
 * project gallery; useful for social/demo purposes).
 */

const WHAT_GAME_SLUG = "rhobh";
...
async function resolveTodaysAnswer(): Promise<string> {
  const game = await getGameBySlug(WHAT_GAME_SLUG);
  if (!game) throw new Error(`Game not found: ${WHAT_GAME_SLUG}`);
...
async function captureScreenshots(opts: Options) {
  ...
  try {
    ...
    await resetBoard(page, `${opts.baseUrl}/games/what`);
    ...
    const gameplayOut = path.join(opts.outDir, "what-gameplay.png");
    ...
    const solvedOut = path.join(opts.outDir, "what-solved.png");
...
async function captureSolveVideo(opts: Options) {
  ...
  const webmOut = path.join(opts.outDir, "what-solve.webm");
