...
async function resolveTodaysAnswer(): Promise<{ answer: string; dateKey: string }> {
  ...
  if (!game) throw new Error(`Game not found: ${DEFAULT_REALITEA_GAME_SLUG}`);
...
async function openGame(page: Page, url: string) {
  ...
  await testId(page, "realitea-tile-grid").waitFor({ timeout: 15_000 });
}

async function typedLetters(page: Page): Promise<string> {
  return page
    .locator('[data-testid="realitea-tile"][data-state="typed"] .realitea-tile-letter')
...
async function waitForError(page: Page, code: string) {
  await page.locator(`[data-testid="realitea-status"][data-error="${code}"]`).waitFor({
    timeout: 8_000,
  });
}
...
async function waitForReveal(page: Page) {
  await page
    .locator(
      '[data-testid="realitea-tile"][data-state="correct"], [data-testid="realitea-tile"][data-state="present"], [data-testid="realitea-tile"][data-state="absent"]',
    )
    .first()
...
async function run(opts: Options) {
  ...
  const gameUrl = `${opts.baseUrl}/games/realitea`;
  ...
  try {
    ...
    probe.check("logo present", await testId(desktop, "realitea-logo").isVisible());
    const iconHrefs = await desktop.locator('link[rel="icon"]').evaluateAll((nodes) =>
      nodes.map((node) => node.getAttribute("href") ?? ""),
    );
    probe.check(
      "RealiTea favicon link present",
      iconHrefs.some((href) => href.includes("logo.realitea.png")),
      iconHrefs.join(", "),
    );
    probe.check(
    ...
    const tileCount = await testId(desktop, "realitea-tile").count();
    probe.check("grid is 6×5", tileCount === 30, `got ${tileCount}`);
    probe.check("onscreen keyboard visible", await testId(desktop, "onscreen-keyboard").isVisible());
    probe.check("history control present", await testId(desktop, "realitea-history-link").isVisible());
    const historyRole = await testId(desktop, "realitea-history-link").getAttribute("role");
    ...
    probe.check(
      ...
      await testId(desktop, "realitea-auth-required").isVisible(),
    );
    probe.check(
    ...
    console.log("\n[history — guest]");
    await testId(desktop, "realitea-history-link").click();
    await testId(desktop, "realitea-history-guest").waitFor({ timeout: 8_000 });
    ...
    const correctCount = await solve
      .locator('[data-testid="realitea-tile"][data-state="correct"]')
      .count();
    probe.check(
    ...
    const resultBadge = testId(solve, "realitea-result-badge");
    probe.check(
      "solved badge is present",
      (await resultBadge.getAttribute("data-solved")) === "true" &&
        (await resultBadge.getAttribute("data-guesses")) === "1",
    );
    probe.check("receipt is present", await testId(solve, "realitea-receipt").isVisible());
    probe.check("share control is present", await testId(solve, "realitea-share").isVisible());
    probe.check("copy control is present", await testId(solve, "realitea-copy-story").isVisible());
    probe.check(
    ...
    await testId(solve, "realitea-share").click();
    ...
    await testId(solve, "realitea-copy-story").click();
    ...
    await testId(topic, "realitea-topic-select").click();
    const currentSlug = await testId(topic, "realitea-topic").getAttribute("data-slug");
    const nextOption = topic.locator(
      `[data-testid^="realitea-topic-option-"]:not([data-testid="realitea-topic-option-${currentSlug}"])`,
    ).first();
    const nextTestId = await nextOption.getAttribute("data-testid");
    const nextSlug = nextTestId?.replace("realitea-topic-option-", "") ?? "";
    await nextOption.click();
    await topic.waitForURL(new RegExp(`[?&]game=${nextSlug}\\b`));
    probe.check("topic change updates the URL", topic.url().includes(`game=${nextSlug}`), topic.url());
    await topic
      .locator(`[data-testid="realitea-topic"][data-slug="${nextSlug}"]`)
      .waitFor({ timeout: 8_000 });
    probe.check(
      "topic control reflects the new slug",
      (await testId(topic, "realitea-topic").getAttribute("data-slug")) === nextSlug,
      nextSlug,
    );
