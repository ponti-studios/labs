...
/**
 * Captures a consistent reference screenshot of every RealiTea tile state
 * (correct / present / absent / typed / empty) at a set of device viewport
 * sizes, in both light and dark color schemes.
 *
 *   pnpm tsx scripts/realitea-tile-states.ts
 *
...
// [letter, state] pairs, in board order. Covers every RealiTeaTileState.
...
async function injectDemoRow(page: Page) {
  await page.waitForSelector('[data-testid="realitea-tile-grid"]');
  await page.waitForSelector('[data-testid="onscreen-keyboard"]');

  await page.evaluate((tiles) => {
    const grid = document.querySelector('[data-testid="realitea-tile-grid"]');
    if (!grid) throw new Error("realitea-tile-grid not found");
    ...
    if (!firstRow) throw new Error("realitea-tile-grid has no rows");
    firstRow.setAttribute("data-testid", "tile-state-demo");
    firstRow.innerHTML = "";

    for (const [letter, state] of tiles) {
      const tile = document.createElement("div");
      tile.className = "realitea-tile";
      tile.dataset.state = state;
      const span = document.createElement("span");
      span.className = "realitea-tile-letter";
...
async function captureDevice(
  ...
  await page.goto(`${opts.baseUrl}/games/realitea`, { waitUntil: "networkidle" });
