...
export const whatAdminActionKindValues = [
  "preview",
  "generate",
  "publish",
  "replace",
...
export type WhatAdminActionKind = (typeof whatAdminActionKindValues)[number];

// ── what_generation_runs ─────────────────────────────────────────────────
...
export const generationRuns = labs.table(
  ...
  (table) => [
    index("what_generation_runs_topic_created_idx").on(table.gamesTopicId, table.createdAt),
    index("what_generation_runs_date_created_idx").on(table.dateKey, table.createdAt),
  ],
);

// ── what_generation_candidates ───────────────────────────────────────────
...
export const generationCandidates = labs.table(
  ...
  (table) => [
    uniqueIndex("what_generation_candidates_run_ordinal_idx").on(table.runId, table.ordinal),
  ],
);
 ...
 * `WhatGuess` in app/lib/what/core/types.ts, but declared here rather
...
export const puzzleRevisions = labs.table("what_puzzle_revisions", {
  id: serial("id").primaryKey(),
  gamesTopicId: integer("games_topic_id")
    .notNull()
    .references(() => gamesTopics.id, { onDelete: "restrict" }),
...
export const adminActions = labs.table(
  "what_admin_actions",
  {
    ...
    hominemUserId: text("hominem_user_id").notNull(),
    kind: text("kind", { enum: whatAdminActionKindValues }).notNull(),
    gamesTopicId: integer("games_topic_id")
      .notNull()
      .references(() => gamesTopics.id, { onDelete: "restrict" }),
  ...
  (table) => [
    index("what_admin_actions_at_idx").on(table.at),
    index("what_admin_actions_kind_at_idx").on(table.kind, table.at),
    index("what_admin_actions_date_idx").on(table.dateUtc),
  ],
);
