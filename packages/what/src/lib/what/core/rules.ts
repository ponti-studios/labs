import type { GameStatus, LetterState, WhatGuess } from "./types";
...
export const WHAT_ANSWER_LENGTH = 5;
...
export function isGuessLengthValid(value: string): boolean {
  return normalizeGuess(value).length === WHAT_ANSWER_LENGTH;
}

export function hasGuessedWord(guesses: readonly WhatGuess[], word: string): boolean {
 ...
 */
export function getKeyboardState(guesses: readonly WhatGuess[]): Record<string, LetterState> {
...
export function isGuessSolved(guess: WhatGuess): boolean {
  return guess.states.every((state) => state === "correct");
}

export function deriveGameStatus(guesses: readonly WhatGuess[]): GameStatus {
