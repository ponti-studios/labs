...
import { DEFAULT_WHAT_GAME_SLUG } from "../generation/catalog";
...
async function requireGameId(gameSlug = DEFAULT_WHAT_GAME_SLUG): Promise<number> {
...
async function resolveActivePuzzle(
  now: Date,
  timeZone: string,
  gameSlug = DEFAULT_WHAT_GAME_SLUG,
...
export async function loadActivePublicPuzzleWithAttempt(
  ...
  if (user) {
    ...
    if (attemptRow) {
      attempt = { guesses: attemptRow.guesses as WhatGuess[], status: attemptRow.status };
    }
  }
...
export async function loadPuzzleForSpecificDate(
  ...
  return {
    ...
    attempt: attemptRow
      ? { guesses: attemptRow.guesses as WhatGuess[], status: attemptRow.status }
      : null,
  };
}
...
export async function evaluateGuessServer(
  ...
  if (word.length !== WHAT_ANSWER_LENGTH) {
  ...
  if (user) {
    ...
    if (attempt) {
      ...
      if (hasGuessedWord(attempt.guesses as WhatGuess[], word)) {
  ...
  if (!attempt) {
    try {
      ...
      attempt = await loadAttempt(user.id, gameId, resolvedDateKey);
      if (!attempt) throw new Error("Failed to create or load what attempt");
    }
  }

  const guessCount = attempt.guesses.length + 1;
  const isGameOver = isSolved || guessCount >= MAX_GUESSES;
  const status: WhatGuessResult["status"] = isSolved
    ? "solved"
