...
import { loadAdminDate } from "~/lib/what/admin/inventory";
...
export function meta() {
  return [{ title: "What date" }, { name: "robots", content: "noindex" }];
}
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  if (!detail) throw Response.json({ error: `No active What topic found` }, { status: 404 });
  return detail;
}

export default function WhatAdminDate() {
  const detail = useLoaderData<typeof loader>();

  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 p-6">
      <Button asChild variant="ghost" size="sm" className="w-fit">
        <Link to={`/games/what/admin?game=${detail.game.slug}`}>← Inventory</Link>
      </Button>
      ...
      <SectionIntro
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button asChild>
              <Link to={`/games/what/admin/generate?game=${detail.game.slug}`}>Generate</Link>
            </Button>
          </div>
        }
      />
